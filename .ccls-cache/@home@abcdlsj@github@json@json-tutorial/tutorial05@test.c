...
static void test_parse_array() {
    ...
    EXPECT_EQ_INT(LEPT_FALSE, lept_get_type(lept_get_array_element(&v,2)));
    ...
    for (i = 0; i < 4; i++) {
        lept_value* x = lept_get_array_element(&v, i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(x));
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(x));

        for (j = 0; j < i; j++) {
            lept_value* e = lept_get_array_element(x, j);
